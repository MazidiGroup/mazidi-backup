...
export async function overview() {
  ...
  const [companies, queue, activity, cfg, leads] = await Promise.all([
    db.from('companies').select('pipeline_status, website, is_customer, notes'),
    db.from('replies').select('reply_id, received_at, from_email, subject, classification, summary, company_id, full_message')
  ...
  return {
    total: rows.length,
    byStatus: by,
    needsReview: rows.filter(r => (!r.website || /NEEDS_REVIEW/.test(r.notes || '')) && !['DO_NOT_CONTACT', 'LOST'].includes(r.pipeline_status)).length,
    qualified: by.QUALIFIED ?? 0,
    customers: rows.filter(r => r.is_customer).length,
    queue: (queue.data ?? []).map(r => ({ ...r, excerpt: (r.full_message || '').replace(/^Message-ID:.*\n\n/, '').split(/\n(?:On .+wrote:|>)/)[0].trim().slice(0, 200), full_message: undefined })),
...
export async function companies(view = 'all') {
  ...
  if (view === 'review')    q = q.or('website.is.null,notes.ilike.%NEEDS_REVIEW%').not('pipeline_status', 'in', '("DO_NOT_CONTACT","LOST")');
  const { data } = await q;
  return data ?? [];
}
...
export async function companyDetail(id) {
  const db = serverClient();
  const [c, contacts, log, outreach] = await Promise.all([
    ...
    db.from('outreach').select('outreach_id, sequence_step, subject, body, delivery_status, sent_at, next_action_due, replied').eq('company_id', id).order('sequence_step')
  ]);
  ...
  return { company: c.data, contacts: contacts.data ?? [], log: [...(log.data ?? []), ...discovery], outreach: outreach.data ?? [] };
}
