...
export function employeeBand(accountsCategory) {
  switch (accountsCategory) {
    ...
    // The small-companies regime: audit exempt, under 50 staff. Most firms of
    // 5-20 people file one of these rather than micro-entity accounts.
    case 'small':
    case 'total-exemption-small':
    case 'total-exemption-full':
    case 'unaudited-abridged':
    case 'abridged':
    case 'filleted':     return { band: '10-50', estimate: 15, fits: 'maybe' };
...
/**
 * Active officers who are natural persons. Names come from the public
 * register, so they are sourced, not guessed. Corporate officers (a company
 * acting as director) are skipped: there is no person to address.
 */
export async function activeOfficers(companyNumber) {
  const data = await ch(`/company/${encodeURIComponent(companyNumber)}/officers`, { items_per_page: 35 });
  const people = (data.items ?? [])
    .filter(o => !o.resigned_on)
    .filter(o => ['director','llp-member','llp-designated-member','member'].includes(o.officer_role))
    .filter(o => !/^corporate-/.test(o.officer_role) && !o.identification)
    .map(o => {
      // Register format is "SURNAME, Forenames". Keep the first forename only.
      const [surname, forenames = ''] = String(o.name).split(',').map(x => x.trim());
      const firstName = forenames.split(/\s+/)[0] || '';
      const title = firstName ? cap(firstName) : '';
      return {
        firstName: title,
        surname: cap(surname),
        role: o.officer_role === 'director' ? 'Director' : 'Member',
        appointedOn: o.appointed_on ?? null,
        sourceUrl: `https://find-and-update.company-information.service.gov.uk/company/${companyNumber}/officers`
      };
    })
    .filter(p => p.surname && p.firstName);
  // Longest-serving first: most likely to be the owner.
  people.sort((a, b) => String(a.appointedOn ?? '').localeCompare(String(b.appointedOn ?? '')));
  return people;
}

const cap = s => String(s || '').toLowerCase().replace(/(^|[\s'-])([a-z])/g, (m, p, c) => p + c.toUpperCase());
