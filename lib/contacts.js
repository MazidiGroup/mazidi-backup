...
import { fetchPage, strip } from './research.js';
import { activeOfficers } from './companiesHouse.js';
...
// Cloudflare "email-decode": <a data-cfemail="hex"> where byte 0 is the key.
...
/** All addresses visible in a page, including the common ways of hiding them. */
function addressesIn(html) {
  const out = new Set();
  const add = e => { if (e) out.add(e.toLowerCase().replace(/^mailto:/, '')); };
  for (const m of html.match(EMAIL_RE) || []) add(m);
  for (const m of html.matchAll(/data-cfemail="([0-9a-f]+)"/gi)) add(decodeCf(m[1]));
  // "info [at] firm.co.uk", "info (at) firm dot co dot uk"
  const text = strip(html);
  const DOT = /\s*(?:[\[(]\s*dot\s*[\])]|\bdot\b)\s*/gi;
  for (const m of text.matchAll(/([a-z0-9._%+-]+)\s*[\[(]\s*at\s*[\])]\s*((?:[a-z0-9-]+(?:\s*(?:[\[(]\s*dot\s*[\])]|\bdot\b|\.)\s*))+[a-z]{2,})/gi)) {
    add(`${m[1]}@${m[2].replace(DOT, '.').replace(/\s+/g, '')}`);
  }
  for (const m of text.match(EMAIL_RE) || []) add(m);
  return [...out];
}

/** Links on a page that look like a contact or team page. */
function siteLinks(html, origin) {
  const out = new Set();
  for (const m of html.matchAll(/href="([^"#?]+)"/gi)) {
    let u;
    try { u = new URL(m[1], origin); } catch { continue; }
    if (u.origin !== origin) continue;
    if (/contact|get-in-touch|reach-us|about|team|people|who-we-are|staff|partners/i.test(u.pathname)) out.add(u.pathname);
  }
...
export async function collectAddresses(website, domain) {
  ...
  while (queue.length && pagesRead < 12) {
    ...
    if (!page) continue;
    pagesRead++;
    // Follow the site's own contact/about links: they beat guessed paths.
    if (path === '') for (const l of siteLinks(page.html, origin)) if (!visited.has(l)) queue.unshift(l);
    const seen = addressesIn(page.html)
