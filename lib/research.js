...
export function candidateDomains(legalName) {
  ...
  const strong = [...new Set([joined, hyphen].filter(b => b.length >= 4 && b.length <= 40))];
  const weak   = t.length > 1 && first.length >= 4 ? [first] : [];
...
export async function findWebsite({ legalName, postcode }) {
  for (const { domain, weak } of candidateDomains(legalName)) {
