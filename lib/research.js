...
/** Fetch with a reason on failure. Tries the honest UA first, then a browser one, since some hosts block anything else. */
export async function fetchPageDetailed(url, ms = 9000) {
  ...
  for (const ua of [UA, BROWSER_UA]) {
    ...
    try {
      ...
      if (!type.includes('text/html')) { last = `not a web page (${type.split(';')[0] || 'unknown type'})`; continue; }
      const html = (await res.text()).slice(0, 400_000);
      return { page: { finalUrl: res.url, html }, reason: null };
    } catch (e) {
      last = e.name === 'AbortError' ? 'timed out' : (e.cause?.code || e.message || 'network error');
    } finally { clearTimeout(t); }
  }
  return { page: null, reason: last };
}

export async function fetchPage(url, ms = 9000) {
  return (await fetchPageDetailed(url, ms)).page;
}
...
export async function confirmWebsite({ url, legalName, postcode }) {
  ...
  // Try the exact address given, then the site root.
  let fetched = await fetchPageDetailed(parsed.href);
  if (!fetched.page && parsed.href !== parsed.origin + '/') fetched = await fetchPageDetailed(parsed.origin);
  // Broken certificate: try plain http once. We never disable certificate checks.
  if (!fetched.page && /CERT|certificate|SSL|TLS/i.test(fetched.reason || '')) {
    const plain = await fetchPageDetailed(`http://${parsed.host}/`);
    if (plain.page) fetched = plain; else fetched.reason = `${fetched.reason}; plain http also failed`;
  }
  const { page, reason } = fetched;
  if (!page) {
    const blocked = /^HTTP (403|401|429|503)$/.test(reason || '');
    return { ok: false, unreachable: true,
      why: blocked ? `The site refused automated access (${reason}). If you have opened it yourself and it is theirs, tick the box below to record it on your say-so.`
                   : `The site could not be read (${reason}). If it opens for you and it is theirs, tick the box below to record it on your say-so.` };
  }
  const text = strip(page.html);
  const m = isMatch(text, legalName, postcode);
  if (!m.matched) {
    return { ok: false, unproven: true, why: 'The page does not show the company name or its registered postcode. If the name is only in a logo image and you are sure it is theirs, tick the box below to record it on your say-so.' };
  }
