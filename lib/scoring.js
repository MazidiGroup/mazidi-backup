...
export function scoreCompany(c, now = Date.now()) {
  ...
  if (c.placesChecked) {
    if (c.placesFound) {
      ...
      const lastReview = monthsSince(c.placesLatestReviewAt, now);
      const staleReviews = c.staleReviewMonths ?? 36;
      // A stale listing is only evidence when there were enough reviews for
      // the silence to mean something. One old review says nothing.
      const reviews = c.placesRatingCount ?? 0;
      if (reviews >= 3 && lastReview !== null && lastReview > staleReviews) add('no_recent_google_activity', -20);
      else if (reviews >= 3)                                                 add('has_google_reviews', 5);
